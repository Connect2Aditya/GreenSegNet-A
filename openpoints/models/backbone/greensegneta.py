...
class LocalAggregation(nn.Module):
    ...
    def __init__(self,
        ...
        for i in range(len(channels1) - 1):  # #layers in each blocks
            convs5.append(create_convblock1d(channels1[i]*4, channels1[i],
                                             norm_args=norm_args,
                                            act_args=None if i == (
                                                    len(channels1) - 2) and not last_act else act_args,
                                             **conv_args)
        ...
        self.convs5 = nn.Sequential(*convs5)
        self.reduction = reduction.lower()
        self.pool = get_reduction_fn(self.reduction)

        group_args['nsample']=8
        self.vfr=VFR(channels[0],channels[-1],flag=1)
    ...
    def forward(self, pf, pe) -> torch.Tensor:
        ...
        fj1 = fj
        B1, C1, npoint1, nsample1 = fj.shape
        fjj = fj.unsqueeze(1).expand(-1, 3, -1, -1, -1) \
                                * dp.unsqueeze(2)
        fjj = fjj.view(B1, -1, npoint1, nsample1)
        fjj = torch.cat((fjj,pe), dim=1) 
        fjj = self.pool(fjj)
        fjj = self.convs5(fjj)
        f = fjj
...
class VFR(nn.Module): 
    def __init__(self,channel,out_channel,flag=0):
        ...
        self.theta_one=nn.Sequential(nn.Conv2d(channel,channel,1,1,bias=False),nn.BatchNorm2d(channel),nn.ReLU(inplace=True))
        self.theta_two=nn.Sequential(nn.Conv2d(channel,channel,1,1,bias=False),nn.BatchNorm2d(channel),nn.ReLU(inplace=True))
        self.linear=nn.Conv2d(channel,channel,kernel_size=1,bias=False)
        self.flag=flag
        if flag==0:
            self.relu=nn.LeakyReLU(0.2,inplace=True)
            self.lrlu=nn.LeakyReLU(0.2,inplace=True)#leakyrelu is better for scanobjectnn
            self.tf_lin=nn.Sequential(nn.Conv1d(channel*3,channel,kernel_size=1,stride=1,groups=channel,bias=False),nn.BatchNorm1d(channel),nn.LeakyReLU(0.2,inplace=True))
        else:
            self.tf_lin=nn.Sequential(nn.Conv1d(channel*3,channel,kernel_size=1,stride=1,groups=channel,bias=False),nn.BatchNorm1d(channel))
    ...
    def forward(self,position,identity,y):# y b,d,g,1
        ...
        y=self.relu(y)
        del position
        theta_one=self.theta_one(y)
        theta_two=self.theta_two(y)
        lin=self.linear(y)
        del y
        lin=torch.cat([lin*torch.sin(theta_two)*torch.cos(theta_one),lin*torch.sin(theta_two)*torch.sin(theta_one),lin*torch.cos(theta_two)],dim=-2).reshape(b,-1,g,k)#b,out*3,g

        del theta_one
        del theta_two
        if self.flag==1:
            lin=torch.sum(lin,dim=-1,keepdim=False)
        else:
            lin=torch.max(lin,dim=-1,keepdim=False)[0]
        
        lin=self.tf_lin(lin)#b,d,g,1
...
class GreenblockA(nn.Module):
    ...
    def forward(self, pf_pe):
        p, f, pe = pf_pe
        identity = f
        f = self.convs([p, f], pe) #Local Aggregation
        f = self.pwconv(f)
...
class GreenSegNetAEncoder(nn.Module):
    r"""The Encoder for GreenSegNet-A
    Args:
        ...
        block (strorType[GreenblockA], optional): the block to use for depth scaling. Defaults to 'GreenblockA'.
        ...
        expansion (int, optional): the expansion ratio of the GreenblockA block. Defaults to 4.
        sa_layers (int, optional): the number of MLP layers to use in the SA block. Defaults to 1.
        sa_use_res (bool, optional): wheter to use residual connection in SA block. 
    """
    ...
    def __init__(self,
                 ...
                 block: str or Type[GreenblockA] = 'GreenblockA',
